...
DATA_DIR = Path("data/raw/tiny")
FILES = {
    "task": "Task_tiny_dense.csv",
    "slot_conflict": "SlotConflict_tiny.csv",
    "instructor_slot": "InstructorSlot_tiny.csv",
    "instructor_skill": "InstructorSkill_tiny.csv",
    "instructor_quota": "InstructorQuota_tiny.csv"
}
OUTPUT_PATH = Path("data/processed/tiny_graph_dense.pickle")


def load_tiny_data(data_dir: Path):
...
def build_graph_from_tiny():
    data = load_tiny_data(DATA_DIR)
...
if __name__ == "__main__":
    try:
        G, raw_data = build_graph_from_tiny()

        print("\n=== Graph Construction Verification ===")
        print(f"Number of Nodes: {G.number_of_nodes()} (Should be 5)")
        ...
        print(f"\nError: {e}")
        print("Please ensure your CSV files are in 'data/raw/tiny/' and filenames match the CONFIGURATION.")
